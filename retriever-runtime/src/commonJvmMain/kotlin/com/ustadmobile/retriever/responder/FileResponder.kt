...
import com.ustadmobile.retriever.util.MimeUtilCommon
import com.ustadmobile.retriever.util.RangeResponse
import com.ustadmobile.retriever.util.parseRangeRequestHeader
    ...
    companion object {
        ...
        fun newResponseFromFile(method: NanoHTTPD.Method, uriResource: RouterNanoHTTPD.UriResource, session: NanoHTTPD.IHTTPSession, file: IFileSource, cacheControlHeader: String? = "cache, max-age=86400"): NanoHTTPD.Response {
            ...
            try {
                ...
                val fileName: String = file.name ?: throw IllegalArgumentException("file has no name!")
                val etagNameInput = fileName
                val fileExtension = session.uri.substringAfterLast('.')
                val mimeType = MimeUtilCommon.getMimeTypeFromExtension(fileExtension)
